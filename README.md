...
`master-data.md` and `applications/log.yaml` are **not tracked on industry branches at
all** — `cv-new-industry` removes the inherited copies when it creates a branch, and
every skill that needs the experience bank reads it with `git show main:master-data.md`
rather than as a path. The skills that touch the application log check out `main` first,
so they read it as a path there. This is deliberate. A tracked copy on each branch drifts behind `main` as your experience
